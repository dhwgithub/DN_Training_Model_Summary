...
class holistic(Model):
    def __init__(self, sub_holistic):
        super(holistic, self).__init__()
        self.sub_holistic = sub_holistic
...
def train_holistic_model():
    ...
    # 优化器设置
    LEARNING_RATE = 0.001
    DECAY_STEPS = TRAIN_NUM // BATCH_SIZE  # 表示多少batch衰减一次
    DECAY_RATE = 0.96


    print('===============================================  数据集准备  ===============================================')
    TRAIN_DATA_PATH = r'.\datasets\all\rvl_cdip_train.tfrecords'
    TEST_DATA_PATH = r'.\datasets\all\rvl_cdip_test.tfrecords'
    VAL_DATA_PATH = r'.\datasets\all\rvl_cdip_val.tfrecords'

    train_data = get_datasets.get_dataset_by_tfrecords(TRAIN_DATA_PATH)
    val_data = get_datasets.get_dataset_by_tfrecords(VAL_DATA_PATH)
    test_data = get_datasets.get_dataset_by_tfrecords(TEST_DATA_PATH)
    ...
    import sub_holistic
    sub_holistic = sub_holistic.sub_holistic()
    model = holistic(sub_holistic)
    checkpoint_path = r"./model/Holistic-model/Holistic.ckpt"
    my_tools.save_and_load_models(checkpoint_path, log_file, model, 'holistic')

    info = '当前模型（.ckpt）保存位置：' + checkpoint_path + '\n'
    log_file.write(info)
    ...
    # 训练过程中保存模型
    cp_callback = my_tools.save_model_checkpoint(checkpoint_path)
    ...
    print('===============================================  可视化模型  ===============================================')
    loss__and_acc_img_path = r'./holistic_loss_and_acc_model.jpg'
    # 可视化并保存图片
