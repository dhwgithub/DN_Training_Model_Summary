...
输入图像尺寸：224x224x3
VGG16-LeftBody：在RVL-CDIP数据集的特定图像区域进行训练（4 epoch）得到，其初始权重来自于VGG16-Holistic模型
"""
...
class Holistic(Model):
    ...
    def call(self, x):
        ...
        # x = self.f3(x)
        return x
# VGG16_Holistic = tf.keras.Sequential([
#     tf.keras.applications.VGG19(input_shape=img_shape, include_top=False, weights='imagenet'),
...
################  保存模型以及可视化  ###################
checkpoint_save_path = r"./model/VGG16-LeftBody-model/VGG16-LeftBody.ckpt"
